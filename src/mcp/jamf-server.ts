...
function createJamfMcpServer(roles: string[], caller: string): McpServer {
        ...
        server.registerTool(
            "jamf_assign_computers_to_prestage",
            {
                description:
                    ...
                    "Safely merges with the prestage's existing scope — existing serials already scoped there are " +
                    "left untouched. Does NOT un-scope a serial from any OTHER prestage it may currently belong to. " +
                    "Accepts the prestage by its displayName (e.g. \"Classroom\") or numeric ID — use " +
                    "jamf_list_prestage_configs to see available prestages. Adds one serial per request rather than " +
                    "one atomic batch, so a serial Jamf's ADE validation rejects (most commonly " +
                    "DEVICE_DOES_NOT_EXIST_ON_TOKEN — its Apple Business Manager assignment has moved off this MDM " +
                    "server, an ABM-side state, not a bug) only fails itself; check the `failed` list in the result " +
                    "rather than assuming an overall success means every serial was added.",
                inputSchema: {
                ...
                try {
                    ...
                    const text = toText(result, response_format, () => {
                        ...
                        if (result.failed.length) {
                            lines.push(`- ⚠️ Failed (${result.failed.length}):`);
                            for (const f of result.failed) lines.push(`  - ${f.serial}: ${f.error}`);
                        }
        ...
        server.registerTool(
            "jamf_list_policies",
            {
                description:
                    ...
                    "and/or by category (server-side filtered, cheap even on a large fleet). Returns policy ID and " +
                    "name. Use jamf_get_policy to see full details for a specific policy, or " +
                    "jamf_list_self_service_policies to narrow down to real Self Service catalog entries " +
                    "specifically — this tool's raw list includes legacy/imaging-era auto-generated policies too.",
                inputSchema: {
                    name: z.string().optional().describe("Optional name filter (case-insensitive substring)"),
                    categoryName: z.string().optional().describe("Optional category name filter (exact match, server-side) — see jamf_list_categories"),
                    excludeAutoGeneratedNames: z
                        .boolean()
                        .default(false)
                        .describe(
                            'Exclude legacy/imaging-era auto-generated policies named like "2014-05-27 at 1:47 PM | ' +
                            'akhazaee | 1 Computer" (timestamp | admin username | computer count) — these are not ' +
                            "real Self Service or automation policies, just old one-off imaging artifacts."
                        ),
                    page: z.number().int().min(0).default(0).describe("Page number (0-indexed)"),
                    pageSize: z.number().int().min(1).max(200).default(100).describe("Results per page (default: 100)"),
                    response_format: ResponseFormatSchema,
            ...
            async ({ name, categoryName, excludeAutoGeneratedNames = false, page = 0, pageSize = 100, response_format = "markdown" }) => {
                try {
                    const data = await client.getPolicies(name, page, pageSize, { categoryName, excludeAutoGeneratedNames });
    ...
    // ── 16b. jamf_list_self_service_policies ─────────────────────────────────
    if (hasRole(roles, JAMF_READ)) {
        server.registerTool(
            "jamf_list_self_service_policies",
            {
                description:
                    "Find policies with Self Service actually enabled (self_service.use_for_self_service), unlike " +
                    "jamf_list_policies which returns every policy including legacy/imaging-era auto-generated ones " +
                    "the Classic API's bulk list endpoint can't filter out server-side. Narrows first for free " +
                    "(optional category filter, auto-generated-name exclusion), then hydrates each remaining " +
                    "candidate individually to check its real Self Service status — capped at maxDetailChecks since " +
                    "hydrating the full policy fleet one-by-one isn't practical in a single call; check `droppedCount` " +
                    "in the result to see if candidates were left unchecked. Also reports whether each match has a " +
                    "Self Service icon set.",
                inputSchema: {
                    categoryName: z.string().optional().describe("Optional category name filter (exact match, server-side) — see jamf_list_categories"),
                    name: z.string().optional().describe("Optional name filter (case-insensitive substring)"),
                    maxDetailChecks: z
                        ...
                        .max(1000)
                        .default(300)
                        .describe("Cap on how many narrowed-down candidates get individually checked for real Self Service status"),
                    response_format: ResponseFormatSchema,
                },
                annotations: { readOnlyHint: true, openWorldHint: true },
            },
            async ({ categoryName, name, maxDetailChecks = 300, response_format = "markdown" }) => {
                try {
                    const result = await client.listSelfServicePolicies({ categoryName, name, maxDetailChecks });
                    const text = toText(result, response_format, () => {
                        const lines = [
                            `## Self Service policies (${result.matches.length} found, ${result.checked}/${result.totalCandidates} candidates checked)`,
                        ];
                        if (result.droppedCount > 0) {
                            lines.push(`⚠️ ${result.droppedCount} candidates were not checked (maxDetailChecks cap) — narrow with categoryName/name or raise the cap for a complete result.`);
                        }
                        if (result.matches.length === 0) {
                            lines.push("No Self Service policies found among the checked candidates.");
                        } else {
                            for (const m of result.matches) {
                                lines.push(
                                    `- **${m.displayName ?? m.name}** (ID: ${m.id})${m.categoryName ? ` | Category: ${m.categoryName}` : ""} | ` +
                                    `Icon: ${m.hasIcon ? `Yes (${m.iconFilename ?? "set"})` : "None"}`
                                );
                            }
                        }
                        return lines.join("\n");
                    ...
                    const text = toText(policy, response_format, () => {
                        ...
                        const lines = [
                            ...
                            packages.length
                                ...
                                : null,
                            policy.self_service?.use_for_self_service
                                ? `- **Self Service:** ${policy.self_service.self_service_display_name ?? gen.name} | Icon: ${
                                      policy.self_service.self_service_icon?.id
                                          ? `Yes (${policy.self_service.self_service_icon.filename ?? "set"})`
                                          : "None — use jamf_upload_policy_icon to add one"
                                  }`
                                : null,
                        ].filter(Boolean).join("\n");
                        return lines;
    ...
    // ── 29b. jamf_upload_policy_icon ──────────────────────────────────────────
    if (hasRole(roles, JAMF_WRITE)) {
        server.registerTool(
            "jamf_upload_policy_icon",
            {
                description:
                    "Upload a Self Service icon image for an existing JAMF Pro policy (name or numeric ID). " +
                    "Confirmed working via POST /JSSResource/fileuploads/policies/id/{id}. Verifies the icon " +
                    "actually stuck with a follow-up read (Classic API writes here can return a 201 but silently " +
                    "no-op) and retries once automatically before reporting failure — check `success` and " +
                    "`retried` in the result rather than assuming the call worked just because it didn't throw.",
                inputSchema: {
                    policy: z.string().describe("Policy name or numeric ID — use jamf_list_policies to find one"),
                    fileContentBase64: z.string().describe("Icon image bytes, base64-encoded (PNG/JPG, small file)"),
                    fileName: z.string().describe("File name for the upload, e.g. \"icon.png\" (extension matters — Jamf infers content type from it)"),
                    response_format: ResponseFormatSchema,
                },
                annotations: { readOnlyHint: false, openWorldHint: true },
            },
            async ({ policy, fileContentBase64, fileName, response_format = "markdown" }) => {
                try {
                    assertRole(roles, JAMF_WRITE);
                    const result = await client.uploadPolicyIcon(policy, fileContentBase64, fileName);
                    const text = toText(result, response_format, () =>
                        result.success
                            ? `## Icon uploaded for **${result.policyName}** (ID: ${result.policyId})\n- Icon: ${result.iconFilename ?? "set"} (ID ${result.iconId})${result.retried ? "\n- Note: first upload attempt silently didn't stick; a retry succeeded." : ""}`
                            : `## ⚠️ Icon upload for **${result.policyName}** (ID: ${result.policyId}) did not stick\nBoth the initial attempt and one retry returned success but a follow-up read shows no icon set. Re-check with jamf_get_policy, and consider retrying this call again.`
                    );
                    return { content: [{ type: "text", text }] };
                } catch (err) {
                    return errorResult(err);
